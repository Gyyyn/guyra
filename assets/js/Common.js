...
export function GuyraFetchData(args={}, url, name, expires) {
  ...
  return new Promise(resolve => {
    ...
    if (localStorage && (localStorage.expires > now) && !args.force) {
      ...
      .then(theData => {
        ...
        expires = now.setMinutes(now.getMinutes() + expires);
  ...
  return new Promise((resolve) => {
    ...
    i18nPromise.then(res => {
      ...
      userDataPromise.then(res => {
        ...
        levelmapPromise.then(res => {

          output.levelmap = res;
          theLevelMap = res.levelmap;
...
export function vibrate(ms, fakeClick) {
